...
# =========================================================================
# 3. Image Padding & Letterbox
# =========================================================================

def _parse_pad_width(pad_width: Union[int, Tuple[int, int], Tuple[int, int, int, int]]) -> Tuple[int, int, int, int]:
    if isinstance(pad_width, int):
        return pad_width, pad_width, pad_width, pad_width
    ...
    raise ValueError("pad_width must be int, (pad_y, pad_x), or (top, bottom, left, right)")


def pad(
    image: Union[Image.Image, np.ndarray],
    pad_width: Union[int, Tuple[int, int], Tuple[int, int, int, int]],
    fill: Union[int, Tuple[int, int, int]] = 0,
    mode: str = "constant",
) -> Union[Image.Image, np.ndarray]:
    """Thêm viền (padding) cho ảnh."""
    is_pil = isinstance(image, Image.Image)
    arr = to_array(image)
    top, bottom, left, right = _parse_pad_width(pad_width)

    cv_mode_map = {
        "constant": cv.BORDER_CONSTANT,
        "reflect": cv.BORDER_REFLECT,
        "edge": cv.BORDER_REPLICATE,
        "symmetric": cv.BORDER_REFLECT_101,
    }
    border_mode = cv_mode_map.get(mode.lower(), cv.BORDER_CONSTANT)
    ...
    padded = cv.copyMakeBorder(arr, top, bottom, left, right, border_mode, value=fill_val)

    if is_pil:
        return to_pil(padded)
    return padded
...
def pad_to_shape(
    ...
    """Padding căn giữa ảnh để khớp với kích thước target_shape (height, width)."""
    arr = to_array(image)
    h, w = arr.shape[:2]
    th, tw = target_shape

    if h > th or w > tw:
        raise ValueError(f"Image shape ({h}, {w}) is larger than target_shape ({th}, {tw})")
    ...
    return pad(image, (top, bottom, left, right), fill=fill, mode="constant")


def letterbox(
    image: Union[Image.Image, np.ndarray],
    target_size: Tuple[int, int] = (640, 640),
    fill: Union[int, Tuple[int, int, int]] = 114,
) -> Tuple[Union[Image.Image, np.ndarray], Tuple[float, float], Tuple[int, int]]:
    """Resize giữ nguyên tỉ lệ khung hình và padding viền theo định dạng YOLO Letterbox."""
    is_pil = isinstance(image, Image.Image)
    arr = to_array(image)
    h, w = arr.shape[:2]
    th, tw = target_size
    ...
    resized = cv.resize(arr, (nw, nh), interpolation=cv.INTER_LINEAR)
    ...
    padded = pad(resized, (top, bottom, left, right), fill=fill, mode="constant")

    if is_pil:
        padded = to_pil(padded)

    return padded, (r, r), (left, top)


# =========================================================================
# 4. Kernel Operations & Filtering
# =========================================================================
...
def filter2d(
    ...
    """Áp dụng ma trận kernel 2D tùy chỉnh lên ảnh."""
    is_pil = isinstance(image, Image.Image)
    arr = to_array(image)
    k = np.array(kernel, dtype=np.float32)

    filtered = cv.filter2D(arr, -1, k)
    if is_pil:
        return to_pil(filtered)
    return filtered
...
def gaussian_blur(
    ...
    blurred = cv.GaussianBlur(arr, (kernel_size, kernel_size), sigmaX=sigma)
    if is_pil:
...
def sharpen(
    ...
    """Làm sắc nét ảnh bằng ma trận kernel sharpen."""
    k = np.array([
        [0, -1, 0],
        [-1, 4 + factor, -1],
        [0, -1, 0]
...
def edge_detection(
    ...
    """Trích xuất đường viền bằng thuật toán Sobel, Laplacian, hoặc Canny."""
    is_pil = isinstance(image, Image.Image)
    arr = to_array(image)
    ...
    if m == "sobel":
        ...
        if max_val > 0:
            edges = (magnitude / max_val * 255).astype(np.uint8)
        else:
        ...
        raise ValueError(f"Unsupported edge detection method: {method!r}. Expected 'sobel', 'laplacian', or 'canny'.")
